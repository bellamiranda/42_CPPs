...
/* 
FLOATING-POINT (precision errors (rounding), but more flexible)
...
pros: uses dynamic precision (adapts to different values)
	can store very large and very small numbers
		but not all decimal numbs are stored exactly - small rouding errors
cons: imprecise due to rouding errors
	more complex and slower in calculations

FIXED-POINT (more precise but limited in range)
...
float	|	scientific notat|	approximate	|	large	| 3.14159, 0.1, -2.5 |
